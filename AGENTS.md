...
このファイルは、本リポジトリで作業するときに Codex (またはレビュアー) が参照する運用ルールをまとめたものです。
...
```powershell
npm test            # node --test の単体テスト (Codex/Codex provider, CLI 探索)
...
## AI 相互レビュー（Codex ↔ Codex）

このリポジトリは汎用ツール [ai-cross-review](https://github.com/ktysne/ai-cross-review) を組み込んでいる（そのままコピーして使う vendoring 方式）。実装が一区切りしたら、**実装 → レビュー → 指摘対応 → 妥当性確認** の往復を Codex / Codex を入れ替えて回す。
