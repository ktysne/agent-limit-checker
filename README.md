...
refresh token 自体の期限 (約 30 日) が切れたときだけ `claude login` による再ログインが必要です。
...
## トラブルシュート
- **Claude が「401 Unauthorized」になる** → refresh token の期限切れです。`claude login` を再実行すると、次回ポーリングで更新後の credentials を読み直します。
